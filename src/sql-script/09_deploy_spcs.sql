-- CPG Brand Product Launch Demo - SPCS Deployment
-- Deploys the React dashboard to Snowpark Container Services

-- Prerequisites:
-- 1. Docker image built and pushed:
--    docker build --platform linux/amd64 -t <registry>/sku-launch-dashboard:latest .
--    docker push <registry>/sku-launch-dashboard:latest
...
spec:
  containers:
    - name: app
      ...
      env:
        ...
        SNOWFLAKE_WAREHOUSE: "SKU_LAUNCH_WH"
  ...
  endpoints:
    - name: app
      ...
      public: true
$$
  EXTERNAL_ACCESS_INTEGRATIONS = (SKU_LAUNCH_EAI)
  MIN_INSTANCES = 1
  MAX_INSTANCES = 1;
